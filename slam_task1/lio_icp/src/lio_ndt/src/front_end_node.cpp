            ...
            while (cloud_data_buff.size() > 0 && imu_data_buff.size() > 0 && gnss_data_buff.size() > 0)
            {
                ...
                if (run_time > 280.0 && !has_global_map_published)
                {
                    if (front_end_ptr->GetNewGlobalMap(global_map_ptr)) 
                    {
                        ...
                        std::cout << "All memory has been released, but the program will continue running to keep RViz open." << std::endl;
                        ...
                        imu_sub_ptr.reset();
                        gnss_sub_ptr.reset();
                        imu_to_lidar_ptr.reset();
                        cloud_pub_ptr.reset();
                        local_map_pub_ptr.reset();
                        global_map_pub_ptr.reset();
                        laser_odom_pub_ptr.reset();
                        gnss_pub_ptr.reset();
                        front_end_ptr.reset();
                        local_map_ptr.reset();
                        global_map_ptr.reset();
                        current_scan_ptr.reset();
                        break;
                    }
                }
            }

...
int main()
{
    ...
    // =======================   optimized icp   =======================
    // 补全相关代码
    OptimizedICPGN optimized_icp; // 创建优化 ICP 对象
    optimized_icp.SetTargetCloud(cloud_target_ptr); // 设置目标点云

    // 设置优化 ICP 的参数
    optimized_icp.SetMaxIterations(30); // 设置最大迭代次数
    optimized_icp.SetMaxCorrespondDistance(0.3); // 设置最大对应点距离
    optimized_icp.SetTransformationEpsilon(1e-4); // 设置收敛条件
